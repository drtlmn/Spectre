...
make.autograph <- function(dat,
  ...
  ## COLOUR CONTROL
  if (!is.null(color_map) && !is.null(fill_map)) {
    p <- p + ggplot2::scale_fill_manual(name = colour.by, values = fill_map)
    p <- p + ggplot2::scale_color_manual(name = colour.by, values = color_map)
  } else {
  ...
  ## THEMES
  ppt_base_size <- 20
  p <- p + ggthemes::theme_tufte(base_size = ppt_base_size, base_family = "sans")

  p <- p + ggplot2::theme(
    axis.title = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.2)),
    axis.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.1)),
    plot.title = ggplot2::element_text(size = ggplot2::rel(1.0), face = "plain", hjust = 0.5),
    axis.line = ggplot2::element_line(color = "black", linewidth = 0.75),
    axis.ticks = ggplot2::element_line(color = "black", linewidth = 0.75),
    legend.position = "bottom",
    legend.title = ggplot2::element_blank(),
    legend.text = ggplot2::element_text(size = ggplot2::rel(1.1)),
    strip.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.1)),
    aspect.ratio = 0.8 # Slightly wider than flow plots for time series
  )
